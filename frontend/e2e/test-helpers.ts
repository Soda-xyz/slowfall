...
export async function openTab(page: Page, tabName: string) {
	...
	await page
		.getByRole("heading", { name: tabName === "Database" ? "Database Control" : tabName })
...
export async function fillPersonForm(
	page: Page,
	{ name, weight, email }: { name: string; weight: string; email: string },
) {
...
export async function fillJumpForm(
	page: Page,
	_opts: { date: string; time: string; altitude: string; craft?: string },
) {
	throw new Error(
		"fillJumpForm helper has been removed because the jump-form e2e spec was archived. Restore from e2e/backup if needed.",
	);
}

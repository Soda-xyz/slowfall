...
const createdPerson = { id: 123, firstName: 'Jane', lastName: 'Doe', email: 'jane@example.com', weight: 72 };
...
test('creates a person via Database Control form', async ({ page }) => {
  ...
  await page.route('**/api/person', async (route) => {
    ...
    if (route.request().method() === 'POST') {
      await route.fulfill({ status: 200, body: JSON.stringify(createdPerson), headers: { 'Content-Type': 'application/json' } });
      return;
    }
